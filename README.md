...
| `dynamic_color` | ^1.8.1 | 动态取色（Material You）|
...
### 🚀 快速开始
1. 下载并安装 APK 文件
2. 打开应用，进入底部导航栏的"设置"页面
3. 点击"导入课表"，选择"普通登录"或"VPN登录"
4. 在内置浏览器中登录（普通：`i.jlu.edu.cn`；VPN：`vpn.jlu.edu.cn`）
5. 登录后找到"我的课表"页面，点击右上角下载按钮提取数据
6. 在弹出的对话框中确认学期并保存
7. 课表自动导入，返回"课程表"页面查看
...
1. **🌐 WebView 登录代理**
   ...
   - 支持普通登录（`i.jlu.edu.cn`）和VPN登录（`vpn.jlu.edu.cn`）两种入口

2. **💉 JavaScript 注入提取**
   - 用户在内置浏览器中登录并打开"我的课表"页面
   - 点击应用栏右上角的下载按钮，触发预编写的 JS 脚本执行
...
4. **📱 数据回传与持久化**
   ...
   - 支持多学期数据管理，按学期分别存储
   - SharedPreferences 本地缓存，自动触发 UI 热重载刷新
